...
flowchart TD
  ...
  X -->|No| LIM{Iteration limit reached?}:::dec
  LIM -->|Yes| E

  LIM -->|No| H{Heading needs fix?<br/>Heading error exceeds ENTER_ANG}:::dec
  H -->|Yes| H1["turnTo(targetHead)"]:::proc --> R
  H -->|No| R["Recompute error"]:::proc

  R --> B{Target behind robot?}:::dec
  B -->|Yes| BK["drive backward (BACKUP_M)"]:::proc --> LAT
  B -->|No| LAT["Lateral check"]:::proc

  LAT --> LX{Lateral error exceeds ENTER_POS?}:::dec
  LX -->|Yes| SL["turnBy ±90"]:::proc --> SD["drive(stepX)"]:::proc --> RB["turnTo(targetHead)"]:::proc --> FWD
  LX -->|No| FWD["Forward check"]:::proc

  FWD --> LY{Forward error exceeds ENTER_POS?}:::dec
